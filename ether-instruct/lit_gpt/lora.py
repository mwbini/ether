    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        if r > 0 and any(enable_lora):
            ...
            self.qkv_shapes = [s for s in qkv_shapes if s] #> (2048, 256) - 8 q and 1 v
            self.lora_B = nn.Parameter(torch.zeros(sum(self.qkv_shapes), r))  # (256, 2)) #> (2048+256, 8)
            ...
            q_per_kv = self.n_head // self.n_query_groups  #> number of queries for each key and value (standard: 1, gemma:8)
            total_qkv = q_per_kv + 2
            head_size = out_features // (self.n_query_groups * total_qkv) #> 2560 (like B, plus the k dim that is padded with 0s)
