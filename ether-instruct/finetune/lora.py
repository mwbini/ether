...
def setup(
    ...
    settings = f'lora-r{lora_r}-a{lora_alpha}_'
    settings += lora_query*'q' + lora_key*'k' + lora_value*'v' + lora_projection*'p' + lora_mlp*'m' + lora_head*'h' + '_'
    settings += f'lr{format(train.learning_rate, ".0e")}_'.replace('0','')
    settings += f'bs{train.global_batch_size}-{train.micro_batch_size}_ep{train.epochs}-{train.epoch_size//1000}k_dr{lora_dropout}_wd{train.weight_decay}_wu{train.lr_warmup_steps}'
    ...
    if sample_with_replacement:
        out_dir_model += '_SWR'
    io.out_dir = io.log_dir / out_dir_ckpt / out_dir_ft / out_dir_model #os.path.join('.logs',out_dir_middle,out_dir_name)
    # makedir
...
def get_batch(
    ...
    if pop:
        ...
        for _ in range(micro_batch_size):
            ...
            #input_ids_labels = [data.pop(i) for i in ix]
